.. type:: mapping
  literal constructor: ([])
  one column: ([key1, ..., keyN])
  two columns: ([key1: value1, key2: value2, ...])
  multiple columns: ([key1: value1a; ... valueNa, key2: value2a; ... valueNa])
  :highlighters: \(\[
    \]\)

  .. could have a "literal" option for the type directive?

.. guide:: A step-by-step introduction to mappings
  ...
  .. subtopic:: What are mappings good for?
  ...
  .. subtopic:: How do I create a mapping?
    ...
    Another method is to use :efun:`mkmapping`. This efun gets two arguments with the first beeing an array of keys and the following beeing arrays of values::
  ...
  .. subtopic:: How can I modify the data of a mapping?
    ...
    Deletion of a key is done with the :operator:`-=` or :efun:`m_delete`. A mapping can only be substracted by one without any values::
  ...
  .. subtopic:: How can I access the data stored in a mapping?
  ...
  .. subtopic:: How can I test for the existance of a key?
  ...
  .. subtopic:: How can I copy a mapping?
  ...
  .. subtopic:: How can I determine the size of a mapping?

    Because a mapping is a kind of rectangle it has two sizes: a length and a width. There are three different efuns to query these values. The first two are :efun:`sizeof`, which returns the amount of key-value associations (the length of a mapping), and :efun:`widthof`, which returns the number of values per key (the width). The third is the efun :efun:`get_type_info`. :efun:`get_type_info` is meant to be a function to identify a datatype. Its return value is an array of two numerical values. The first specifies the datatype of the argument and the second is a datatype dependend value. In the case of a mapping the first value is :macro:`T_MAPPING` (which is a value defined in <lpctypes.h>) and the second the amount of values per key (a.k.a. columns or the width of the mapping - actually it would be correct to say that the width of a mapping is the amount of columns plus one for the keys but this is uncommon).

  .. subtopic:: What is the best method to iterate over a mapping?
    ...
    Because a function can only return a single value (even when it is an array) it restricts the use of :efun:`map_indices` to only allow creation of mappings with a single value per key.

  .. subtopic:: Is it possible to join/intersect/cut mappings with another?
    ...
    Cutting out all key-value associations of a mapping for which a key could be found in another mapping can be done by using :operator:`-` and :operator:`-=`::
  ...
  .. subtopic:: What are those mappings without any values (besides keys) good for?
    ...
    :efun:`mkmapping` uses *array* to create a mapping without any values but just keys. And because a mapping can only have unique keys all multiple values in *array* are taken as one. The call of :efun:`m_indices` then returns an array of these unique keys. Actually we only make use of those mappings temporarily.

  .. subtopic:: How can I convert an alist into a mapping and vice versa?
  ...
  :history 3.2.9/3.3.208 changed: added ([:width ]) notation
  ...
  .. seealso:: :lang:`alists`, :type:`closure`, :type:`struct`, :efun:`mkmapping`, :efun:`walk_mapping`
