public class efun {
    ...
    * 
    * The efun takes the command string \p cmd and the object(s) \p env/\p arr and tries to match it against the format string \p fmt. Successfully matched elements are assigned to the variables `var`.... The result from the efun is 1 if the command could be fully matched, and 0 otherwise.
    * 
    * If the objects are given as a single object \p env, the efun matches against the given object and all objects contained therein. Otherwise, if the objects are given as an array \p arr of objects, the efun matches only against the given objects.
    * 
    * The format string \p fmt consists of words, syntactic markers, and %-directives for the values to parse and return in the variables. A typical example is `` 'get' / 'take' %i `` or `` 'spray' / 'paint' [paint] %i >>``<<. The elements in detail are:
    * 
    ...
    * 
    * A <em>word</em> in this context is any sequence of characters not containing a space. 'living objects' are searched by calls to the (simul)efuns `find_player()` and `find_living()`: both functions have to accept a name as argument and return the object for this name, or 0 if there is none.
    * 
    ...
    *          In the \p env form this may be the whole
    *          recursive inventory of the \p env object.
    *          
    ...
    * 
    * @par note: Patterns of type: "%s %w %i" might not work as one would expect. %w will always succeed so the arg corresponding to %s will always be empty.
    * 
    ...
    * 
    * @history{
    * changed (3.3.258) -- removed the compat-mode parse_command().
    * }
    * 
    * @see efun::sscanf()
   **/
