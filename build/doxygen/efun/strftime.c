public class efun {
   /**
    * Formats the given time in \p clock as a string, similar to efun::ctime(). Unlike efun::ctime(), efun::strftime() accepts a format string with placeholders for the different elements. The format string consists of zero or more conversion specifiers (see below) and ordinary characters. Ordinary characters are just copied to the result. A conversion specifier starts with a percent sign ('%'). If no format string is given, a default of "%c" will be used.
    * 
    * The argument \p clock is the number of seconds since the epoch (`00:00:00 UTC, January 1, 1970`), as returned by efun::time()/efun::mktime(). If \p clock is not given, the current result of efun::time() will be used.
    * 
    * The argument \p localized specifies whether the result will be in English (the classic "C" locale) or in the language configured on the host computer (e.g. German). The language has to be configured with the environment variables `LC_TIME` or `LC_ALL` (please ask your admins). If \p localized is not given, the default is 1.
    * 0: use classic "C" locale (english)
    * 1: use current locale on the host computer (national representation).
    * 
    * @par note: The resulting string is at most 511 character long.
    * 
    * ## Conversion Specifiers #
    * 
    * This function accepts all conversion specifiers which the function `strftime()` from the C library accepts. Currently these are:
    * 
    ...
    * %E* %O*
    * 
    * POSIX locale extensions. The sequences %Ec %EC %Ex %EX %Ey %EY %Od %Oe %OH %OI %Om %OM %OS %Ou %OU %OV %Ow %OW %Oy are supposed to provide alternate representations. Additionally %OB implemented to represent alternative month names (used standalone, without day mentioned).
    * 
    ...
    * %V    replaced by the week number of the year (Monday as the first day of the week) as a decimal number (01-53). If the week containing January 1st has four or more days in the new year, then it is week 1; otherwise it is the last week of the previous year, and the next week is week 1.
    ...
    * %%    replaced by >>`<<%'.
    * 
    * @par limit: There is no conversion specification for the phase of the moon.
    * 
    * @synopsis{
    * string strftime()
    ...
    * }
    * 
    * @see efun::ctime(), efun::gmtime(), efun::localtime(), efun::mktime(), efun::time(), efun::utime()
   **/
   string strftime(string fmt, int clock, int localized);
}
