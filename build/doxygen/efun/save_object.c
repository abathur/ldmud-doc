public class efun {
    ...
    * 
    * In the first form, the string is written to the file `name`. A suffix ".c" will be stripped from the name, the suffix ".o" may be added by the master object during the check in master::valid_read(). Result is 0 if the save file could be created, and non-zero on a non-fatal error (file could not be written, or current object is destructed).
    * 
    * In the second form the string is returned directly. If the object is destructed, the result is 0.
    * 
    * In both forms, the optional `format` argument determines the format of the savefile to be written:
    * 
    ...
    * 
    * @history{
    * changed (3.2.8) -- now returns a success value.
...
supports direct encoding into a string.
    ...
    * }
    * 
    * @see efun::restore_object(), efun::save_value()
   **/
   string save_object([int format]);
}
