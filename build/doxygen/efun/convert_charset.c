public class efun {
   /**
    * Convert the string \p str from charset \p from_cs to charset \p to_cs and return the converted string.
    * 
    * The charsets for \p from_cs and \p to_cs are given by name, and both lowercase and uppercase are accepted.
    * 
    * The efun is only available on systems with libiconv installed and the actual capabilities / available charsets depend on the host systems's libiconv/libc.
    * 
    * Usually, the following charsets for \p from_cs and \p to_cs are available:
    * 
