public class efun {
   /**
    * This function takes a path as argument and returns an array of file names and attributes in that directory.
    * 
    * Returns 0 if the directory to search in does not exist.
    * 
    * The filename part of the path may contain '*' or '?' as wildcards: every '*' matches an arbitrary amount of characters (or just itself), a '?' matches any character. Thus get_dir("/path/<em>") would return an alphabetically sorted array of all files in directory "/path", or just ({ "/path/</em>" }) if this file happens to exist.
    * 
    * To query the content of a directory, use the directory name with a trailing '/' or '/.', for example `get_dir("/path/.")`. Use the directory name as it is to get information about the directory itself.
    * 
    ...
    * 
    * ~~~
    * @par note: You should use `GETDIR_NAMES|GETDIR_UNSORTED` to get the entries in the same order as with `GETDIR_SIZES` and `GETDIR_DATES`.
    * 
    * The values of mask can be added together.
    * 
    * @par note: The path argument to this efun is processed by master::valid_read() in the mudlib master before being used. The mudlib may normalize this path (e.g. strip leading or trailing "/") and this may lead to unexpected results (e.g. `get_dir("/dir/", ...)` not returning the contents of "/dir/").
    * 
    * @par note: in @ref driver_topics_predefined "__COMPAT_MODE__", `GETDIR_PATH` will return the paths without the leading /.
    * 
    * @headerfile <files.h>
    * 
    * @synopsis{
    ...
    * 
    * ~~~
    * This returns an array of arrays, with filename, size and filetime as elements, not sorted by names:
    * 
    * ~~~{.c}
    ...
    * }
    * 
    * @see efun::mkdir(), efun::rmdir(), efun::file_size(), efun::write_file(), efun::write_bytes(), efun::read_file(), efun::read_bytes()
   **/
   mixed * get_dir(string str, int mask);
}
