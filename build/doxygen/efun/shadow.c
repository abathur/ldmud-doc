...
With the three objects a.c, b.c and c.c
   ...
   int fun() {
      ...
      debug_message(sprintf("%O [b] fun()n", this_object()));
      find_object("a")->fun();

   }
   void fun2() {

      debug_message(sprintf("%O [b] fun2()n", this_object()));
      find_object("a")->fun3();
      this_object()->fun3();
   ...
   int fun() {
      ...
      debug_message(sprintf("%O [c] fun()n", this_object()));
      find_object("a")->fun();
...
code like the following
   ...
   b->do_shadow(a);
   c->do_shadow(a);
   debug_message("--- a->fun() ---n");
   a->fun();
   debug_message("--- b->fun() ---n");
   b->fun();
   debug_message("--- c->fun() ---n");
   c->fun();
   debug_message("--- b->fun2() ---n");
   b->fun2();
...
Up to @subpage driver_mailto:3.2.1@46 "3.2.1@46", destructing a shadowed object also destructs
all shadows. Since @subpage driver_mailto:3.2.1@47 "3.2.1@47", shadows may survive the
...
shadowed with the #pragma no_shadow.
   ...
   See Also: 
      efun::query_shadowing(), efun::unshadow(), master::query_allow_shadow()
