public class applied {
   /**
    * When the driver created a new connection (either by accepting it or by creating it with efun::net_connect()) and bound it to an object, it then calls applied::logon() in that object.
    * 
    ...
    * 
    * If the master attempted a secure connection in master::connect() and did not set an explicit TLS callback, the call to `logon()` won't happen until the TLS handshake is complete. If the master set a TLS callback, that will be executed in place of `logon()`.
    * 
    * @synopsis{
    * int logon(void)
    * int logon(int flag)
