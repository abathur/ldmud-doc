public class applied {
    ...
    * Or more formally:
    * 
    * - If the object O that moves is marked as living then first call @ainit()` of the destination object D with @athis_player()` set to O.
    * - Then apply the two following rules for each object C inside D:
    * - If C is marked as living then call @aO->init()` with @athis_player()` set to C.
    * - If O is marked as living then call @aC->init()` with @athis_player()` set to O.
    * - Finally, if D is marked as living then call @aO->init()`, with @athis_player()` set to D.
    * 
    * @par warning:commands defined in the player object for the player should not be defined in init(), as these commands would be added to _other_ players whenever they happen to be nearby. Instead use a separate function ("add_player_commands()" or so) which is called during the creation of the player.
    * 
    * @synopsis{
    * void init()
    * }
    ...
    * }
    * 
    * @see efun::add_action(), efun::set_environment(), efun::environment(), efun::move_object()
   **/
   void init();
}
