public class applied {
   /**
    * After @aset_modify_command(mob)` was called for an interactive object iob, all commands for that user will be passed to @amob->modify_command(text)`, and the return will then be checked for actions. The actual name of the lfun to call is specified by the H_MODIFY_COMMAND_FNAME hook - traditionally it's 'modify_command'.
    * 
