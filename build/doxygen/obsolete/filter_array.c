public class efun {
    ...
    * 
    * Since @subpage driver_mailto:3.2.1@36 "3.2.1@36", the second arg can also be a mapping. Then
    * only the elements of the array which belong to the map (as
    * keys) will be returned (i.e. map[arr[index]] != 0).
    * 
    * @synopsis{
    * mixed *filter_array(mixed *arr, string fun, string|object ob, mixed extra, ...)
    ...
    * 
    * @history{
    * changed (3.2.6) -- obsoleted by efun filter().
    * changed (3.2.9) -- not available if driver is compiled without USE_DEPRECATED.
    * removed (3.3)
    * }
    * 
    * @see efun::filter(), efun::filter_objects(), efun::map(), efun::map_objects()
   **/
   mixed *filter_array(mixed *arr, mapping map, mixed extra, ...);
}
