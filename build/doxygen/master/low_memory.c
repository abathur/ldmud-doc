public class master {
    ...
    * 
    * - NO_MALLOC_LIMIT_EXCEEDED   (0)No limit was exceeded, but a garbage collection was requested by a
    * call to garbage_collection() and it will begin right afterlow_memory() returns.
    * - SOFT_MALLOC_LIMIT_EXCEEDED (1)The soft limit was exceeded.
    * - HARD_MALLOC_LIMIT_EXCEEDED (2)The hard limit was exceeded. A garbage collection will begin right
    ...
    * limit was hit.
    * 
    * @par note:If in a low_memory condition when the memory reserves have been
    * used already and they could not be re-allocated after a garbage collection
    ...
    * }
    * 
    * @see efun::debug_info(), efun::garbage_collection(), master::slow_shut_down(), @ref driver_concepts_memory "memory"
   **/
   void low_memory(int what, int limitvalue, int memory, int reservestate);
}
