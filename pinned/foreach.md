...
Every `var` specification can declare a new local variable, whose scope is the whole @ref driver_syntax_foreach "foreach" statement.
...
- assignments to single elements or to array ranges effect the values assigned to the variable:~~~{.c}
a = ({1, 2, 3})
foreach(x : a) {
  a[1..2] = ({4, 5});
  write(x+" ");
...
@par warning: The additional syntax forms using "in" as keyword are meant to make re-engineering of MudOS objects easier. Do not use them for newly written code, as they may not be available in future.
