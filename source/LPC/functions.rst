.. concept:: functions
  ...
  .. subtopic:: DEFINING A FUNCTION
    ...
    ``<modifiers>``
      can be any one of "static", "private", "public"
      and "protected" (see :lang:`modifiers`), optionally combined
      with "varargs" (see :lang:`varargs`) and/or "nomask".
    ...
    ``<type>``
      is the type of the result returned by the function.
    ...
    ``name``
      is the name of the function, e.g. "short", or "Nice_Try",
      under which it is made known.

    ``<arguments>``
      is a list of variable definitions in the
      normal ``<type> <name>`` style, separated by comma. Examples:
    ...
    ``{ statements... }``
      defines the code for this function. This
      is a normal block (see :lang:`block`) and as such can define
  ...
  .. subtopic:: CALLING A FUNCTION
    ...
    .. note:: See the entry H_DEFAULT_METHOD in :concept:`hooks` for a modification
      of this behaviour.
