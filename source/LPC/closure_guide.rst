  ...
  .. subtopic:: Lfun- and Lambda-Closures
    ...
    .. subtopic:: Lambda-Closures
      ...
      .. subtopic:: Special Efun-Closures and Operator-Closures for Lambdas
        ...
        ``#'while``
          acts like the LPC statement "while" and repeats executing one
        ...
        ``#'do``
          is like the do-while statement in LPC and is very much like the
        ...
        ``#'foreach``
          is like the foreach() statement in LPC. It evaluates one or
        ...
        ``#'return ``
          gets one argument and acts like the "return" statement in LPC
        ...
        ``#'break``
          is used like the "break" statement in LPC and aborts the execution
          ...
          function the closure points to!::
        ...
        `#'continue`
          is used like the "continue" statement in LPC and jumps to
          the end of the current loop and continues with the loop
          condition.

        ``#'default``
          may be used within a #'switch-construct but be careful!
        ...
        ``#'switch``
          is used to create closures which behave very much like the
        ...
        ``#'&&``
          evaluates the arguments from the first on and stops if one evaluates
          to 0 and returns 0. If none evaluates to 0 it returns the result of
          the last argument.

        ``#'||``
          evaluates the arguments from the first on and stops if one evaluates
          to true (not 0) and returns it. If all evaluate to 0 it returns 0.

        ``#'catch``
          executes the closure given as argument, but catches any
          runtime error (see :efun:`catch`). Optionally the symbols 'nolog,
          'publish and 'reserve may be given as additional arguments to
          modify the behaviour of the catch.

        ``#'sscanf``
          acts similar to how a funcall would, but passes the third
          and following arguments as lvalues, that is, values which can
          be assigned to.

        ``#'=`` and the ``#'<op>=`` variants
          are also special because the first
          argument has to be an lvalue.

    .. subtopic:: Closures with Strange Names
      ...
      ``#'([``
        is used to create mappings out of single entries (with several values)
      ...
      ``#'[,]``
        ...
        key) directly with :efun:`funcall`. Example::
    ...
    .. subtopic:: Variable-Closures
      ...
      Anybody who now calls ``object->foo()`` will get a closure which will,
      when evaluated, write the actual value of object's global variable x.
