.. topic:: `native` driver mode
  ...
  .. todo:: partial rewrite using a few semantic structures, move version notes to a history section?
  ...
  Predefined Preprocessor Symbols
    - If compat, the symbols :macro:`COMPAT_FLAG` and :macro:`__COMPAT_MODE__` are defined for all LPC programs.
    - If strict-euids, the symbol :macro:`__STRICT_EUIDS__` is defined for all LPC programs.
    - For compatibility reasons, the symbol :macro:`__EUIDS__` is defined for all LPC programs all the time.
  ...
  Userids and Effective Userids
    ...
    .. subtopic:: In Detail
      ...
      Userids of New Objects
        To determine the (e)uids for a new object (loaded or inherited, or cloned), the appropriate driver hook is evaluated (:hook:`H_LOAD_UIDS`, :hook:`H_CLONE_UIDS`) and the result set as (e)uid. The result may be a single value, in which case the euid is set to 0, or an array ``({ uid, euid })``.
  ...
  .. seealso:: :overview:`hook`, :concept:`uids`, :efun:`move_object`, :topics:`initialisation`
