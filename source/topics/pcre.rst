.. topic:: Perl-compatible regular expressions
  :name: pcre

  This document describes the regular expressions supported by the PCRE package. When the package is compiled into the driver, the macro :macro:`__PCRE__` is defined.
  ...
  .. subtopic:: Pcre Regular Expression Details
    ...
    The basic operation of PCRE is on strings of bytes. However, there is also support for UTF-8 character strings. To use this support you must build PCRE to include UTF-8 support, and then call pcre_compile() with the :macro:`PCRE_UTF8` option. How this affects the pattern matching is mentioned in several places below. There is also a summary of UTF-8 features in the section on UTF-8 support in the main pcre page.
  ...
  .. subtopic:: Backslash
    ...
    They are not affected by the :macro:`PCRE_NOTBOL` or :macro:`PCRE_NOTEOL` options. If the startoffset argument of ``pcre_exec()`` is non-zero, indicating that matching is to start at a point other than the beginning of the subject, \A can never match. The difference between \Z and \z is that \Z matches before a newline that is the last character of the string as well as at the end of the string, whereas \z matches only at the end.
  ...
  .. subtopic:: Circumflex And Dollar

    Outside a character class, in the default matching mode, the circumflex character is an assertion which is true only if the current matching point is at the start of the subject string. If the startoffset argument of ``pcre_exec()`` is non-zero, circumflex can never match if the :macro:`PCRE_MULTILINE` option is unset. Inside a character class, circumflex has an entirely different meaning (see below).
    ...
    The meaning of dollar can be changed so that it matches only at the very end of the string, by setting the :macro:`PCRE_DOLLAR_ENDONLY` option at compile time. This does not affect the \Z assertion.

    The meanings of the circumflex and dollar characters are changed if the :macro:`PCRE_MULTILINE` option is set. When this is the case, they match immediately after and immediately before an internal newline character, respectively, in addition to matching at the start and end of the subject string. For example, the pattern /^abc$/ matches the subject string "def\nabc" in multiline mode, but not otherwise. Consequently, patterns that are anchored in single line mode because all branches start with ^ are not anchored in multiline mode, and a match for circumflex is possible when the startoffset argument of ``pcre_exec()`` is non-zero. The :macro:`PCRE_DOLLAR_ENDONLY` option is ignored if :macro:`PCRE_MULTILINE` is set.

    Note that the sequences \A, \Z, and \z can be used to match the start and end of the subject in both modes, and if all branches of a pattern start with \A it is always anchored, whether :macro:`PCRE_MULTILINE` is set or not.

  .. subtopic:: Full Stop (period, Dot)

    Outside a character class, a dot in the pattern matches any one character in the subject, including a non-printing character, but not (by default) newline. In UTF-8 mode, a dot matches any UTF-8 character, which might be more than one byte long, except (by default) for newline. If the :macro:`PCRE_DOTALL` option is set, dots match newlines as well. The handling of dot is entirely independent of the handling of circumflex and dollar, the only relationship being that they both involve newline characters. Dot has no special meaning in a character class.

  .. subtopic:: Matching A Single Byte
  ...
  .. subtopic:: Square Brackets
    ...
    The newline character is never treated in any special way in character classes, whatever the setting of the :macro:`PCRE_DOTALL` or :macro:`PCRE_MULTILINE` options is. A class such as [^a] will always match a newline.
  ...
  .. subtopic:: Posix Character Classes
  ...
  .. subtopic:: Internal Option Setting

    The settings of the :macro:`PCRE_CASELESS`, :macro:`PCRE_MULTILINE`, :macro:`PCRE_DOTALL`, and :macro:`PCRE_EXTENDED` options can be changed from within the pattern by a sequence of Perl option letters enclosed between "(?" and ")". The option letters are::
    ...
    For example, (?im) sets caseless, multiline matching. It is also possible to unset these options by preceding the letter with a hyphen, and a combined setting and unsetting such as (?im-sx), which sets :macro:`PCRE_CASELESS` and :macro:`PCRE_MULTILINE` while unsetting :macro:`PCRE_DOTALL` and :macro:`PCRE_EXTENDED`, is also permitted. If a letter appears both before and after the hyphen, the option is unset.
    ...
    matches abc and aBc and no other strings (assuming :macro:`PCRE_CASELESS` is not used). By this means, options can be made to have different settings in different parts of the pattern. Any changes made in one alternative do carry on into subsequent branches within the same subpattern. For example,::

      (a(?i)b|c)

    matches "ab", "aB", "c", and "C", even though when matching "C" the first branch is abandoned before the option setting. This is because the effects of option settings happen at compile time. There would be some very weird behaviour otherwise.

    The PCRE-specific options :macro:`PCRE_UNGREEDY` and :macro:`PCRE_EXTRA` can be changed in the same way as the Perl-compatible options by using the characters U and X respectively. The (?X) flag setting is special in that it must always occur earlier in the pattern than any of the additional features it turns on, even when it is at top level. It is best put at the start.

  .. subtopic:: Subpatterns
  ...
  .. subtopic:: Repetition
    ...
    If the :macro:`PCRE_UNGREEDY` option is set (an option which is not available in Perl), the quantifiers are not greedy by default, but individual ones can be made greedy by following them with a question mark. In other words, it inverts the default behaviour.

    When a parenthesized subpattern is quantified with a minimum repeat count that is greater than 1 or with a limited maximum, more store is required for the compiled pattern, in proportion to the size of the minimum or maximum.

    If a pattern starts with ``.*`` or ``.{0,}`` and the :macro:`PCRE_DOTALL` option (equivalent to Perl's ``/s``) is set, thus allowing the ``.`` to match newlines, the pattern is implicitly anchored, because whatever follows will be tried against every character position in the subject string, so there is no point in retrying the overall match at any position after the first. PCRE normally treats such a pattern as though it were preceded by ``\A``.

    In cases where it is known that the subject string contains no newlines, it is worth setting :macro:`PCRE_DOTALL` in order to obtain this optimization, or alternatively using ^ to indicate anchoring explicitly.
  ...
  .. subtopic:: Atomic Grouping And Possessive Quantifiers
    ...
    Possessive quantifiers are always greedy; the setting of the :macro:`PCRE_UNGREEDY` option is ignored. They are a convenient notation for the simpler forms of atomic group. However, there is no difference in the meaning or processing of a possessive quantifier and the equivalent atomic group.
  ...
  .. subtopic:: CONDITIONAL SUBPATTERNS
    ...
    There are three kinds of condition. If the text between the parentheses consists of a sequence of digits, the condition is satisfied if the capturing subpattern of that number has previously matched. The number must be greater than zero. Consider the following pattern, which contains non-significant white space to make it more readable (assume the :macro:`PCRE_EXTENDED` option) and to divide it into three parts for ease of discussion::
  ...
  .. subtopic:: Comments
    ...
    If the :macro:`PCRE_EXTENDED` option is set, an unescaped # character outside a character class introduces a comment that continues up to the next newline character in the pattern.
  ...
  .. subtopic:: Recursive Patterns
    ...
    For example, this PCRE pattern solves the nested parentheses problem (assume the :macro:`PCRE_EXTENDED` option is set so that white space is ignored)::
    ...
    the string they capture is "ab(cd)ef", the contents of the top level parentheses. If there are more than 15 capturing parentheses in a pattern, PCRE has to obtain extra memory to store data during a recursion, which it does by using pcre_malloc, freeing it via pcre_free afterwards. If no memory can be obtained, the match fails with the :macro:`PCRE_ERROR_NOMEMORY` error.
  ...
  .. subtopic:: Subpatterns As Subroutines
  ...
  .. subtopic:: Differences From Perl
    ...
    8. There are some differences that are concerned with the settings of captured strings when part of a pattern is repeated. For example, matching "aba" against the pattern :regexp:`/^(a(b)?)+$/` in Perl leaves $2 unset, but in PCRE it is set to "b".
    ...
    9. PCRE provides some extensions to the Perl regular expression facilities:
      ...
      b. If :macro:`PCRE_DOLLAR_ENDONLY` is set and :macro:`PCRE_MULTILINE` is not set, the $ meta-character matches only at the very end of the string.

      c. If :macro:`PCRE_EXTRA` is set, a backslash followed by a letter with no special meaning is faulted.

      d. If :macro:`PCRE_UNGREEDY` is set, the greediness of the repetition quantifiers is inverted, that is, by default they are not greedy, but if followed by a question mark they are.

      e. :macro:`PCRE_ANCHORED` can be used to force a pattern to be tried only at the first matching position in the subject string.

      f. The :macro:`PCRE_NOTBOL`, :macro:`PCRE_NOTEOL`, :macro:`PCRE_NOTEMPTY`, and :macro:`PCRE_NO_AUTO_CAPTURE` options for ``pcre_exec()`` have no Perl equivalents.
  ...
  .. seealso:: :concept:`regexp`, :concept:`hsregexp`
