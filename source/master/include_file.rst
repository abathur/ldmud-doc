.. master:: mixed include_file (string file, string compiled_file, int sys_include)
  ...
  Result:
    ...
    ``0``
      use the normal include filename generation (""-includes are
      used as they are, <>-includes are handled according to
      H_INCLUDE_DIRS).

    ``<path>``
      the full absolute pathname of the file to include,
...
.. history
  ...
  .. seealso:: :concept:`hooks`, :master:`inherit_file`, :hook:`include_dirs`
