.. concept:: memory
  ...
  Changed mechanism of calling `clean_up()` slightly. Only objects that define the function will be called. And, only `clean_up()` that returns non-zero will be called again. This will minimize calls of `clean_up()`, while still costing little to maintain.

  `clean_up()` now gets a flag as argument, which will be non-zero if the the program of this object is used for inheritance by other objects.

  .. seealso:: :applied:`clean_up`, :master:`slow_shut_down`, :master:`quota_demon`, :master:`low_memory`, :driver:`malloc`, :efun:`garbage_collection`
