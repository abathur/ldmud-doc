.. topic:: Starting LDMud
  ...
  .. todo:: needs much general/clarity work

  .. todo:: in this document, any references to _compile_ options need to be properly linked/refd; this involves using the option role for something like `configure --enable-yydebug`. Two tricks: create a doc that documents them in the first place, and convert the syntax used here to the actual configure syntax. search for a few keywords like compile to hunt down documentation that refers to configure/build/compile-time options for the driver
