.. guide:: closures
  ...
  .. todo:: most of this doc just uses implicit definition lists, but one section uses the enumerate directive; they should probably all use enumerate, no?
  ...
  .. subtopic:: Overview and efun-closures
    ...
    .. tip:: If your editor gets confused by the hashmark, try putting parentheses around such a closure-notation because otherwise my : (``#'write``). This is especially of interest within lambda-closures (see below).
    ...
    So, for each element of the array returned by :efun:`users` the function ``query_is_wizard`` is called and only those for which 1 was returned are collected into the result and then put into the variable x.
  ...
  .. subtopic:: Lfun- and Lambda-Closures
    ...
    .. subtopic:: Lambda-Closures
      ...
      .. note:: the syntax with the arrow ``->`` for :efun:`call_other` cannot be used, ``#'->`` does not exist. You have to use ``#'call_other`` for this and give the name of the lfun to be called as a string.)
      ...
      .. subtopic:: Advantages of Lambda Closures
      ...
      .. subtopic:: Free Variables in Lambda-Closure Constructs
        ...
        The closure ``#'=`` is used to assign a value to something (like :operator:`=`).

      .. subtopic:: Special Efun-Closures and Operator-Closures for Lambdas
        ...
        ``#'foreach``
          is like the :keyword:`foreach` statement in LPC. It evaluates one or more bodies repeatedly for every value in a giving string, array or mapping. The result of the closure is 0.
        ...
        ``#'..``
          may be used within a ``#'switch-construct`` but is not implemented yet (3.2.1@131). But ``#'[..]`` works well instead of it.
        ...
        ``#'switch``
          ...
          .. todo:: fix this section. probably only a table or raw plaintext can handle this, if even that...
    ...
    .. enumerate:: Closures with Strange Names
